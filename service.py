...
def load_models():
    ...
    bert_lp_ckpt = os.path.expanduser("~/lprobe/BERT/20250226/lightning_logs/version_0/checkpoints/last.ckpt")
    bert_lp = LinearProbe(bert_model)
    bert_lp.load_state_dict(
            torch.load(bert_lp_ckpt, weights_only=True, map_location=torch.device('cpu'))["state_dict"],
            )
    
    ernie_lp_ckpt = os.path.expanduser("~/lprobe/ERNIE/20250226/lightning_logs/version_0/checkpoints/last.ckpt")
    ernie_lp = LinearProbe(ernie_model)
    ernie_lp.load_state_dict(
            torch.load(ernie_lp_ckpt, weights_only=True, map_location=torch.device('cpu'))["state_dict"],
            )
...
def process_models(input_text):
    # Process with BERT
    bert_inputs = models["bert"]["tokenizer"](input_text, return_tensors="pt", padding=True, truncation=True, max_length=512)
    with torch.no_grad():
        bert_outputs = models["bert"]["model"](bert_inputs)
    
    # Process with ERNIE
    ernie_inputs = models["ernie"]["tokenizer"](input_text, return_tensors="pt", padding=True, truncation=True, max_length=512)
    with torch.no_grad():
        ernie_outputs = models["ernie"]["model"](ernie_inputs)
    
    # Format results according to required structure
    results = {
        "BERT": {f"l{i+1}": float(F.sigmoid(z).item()) for i, z in enumerate(bert_outputs)},
        "ERNIE": {f"l{i+1}": float(F.sigmoid(z).item()) for i, z in enumerate(ernie_outputs)}
    }
